...
def get_main_menu(user_id):
    keyboard = [
        ...
        [KeyboardButton(text="⏰ Напоминание")]  # Новая кнопка
    ]
...
async def check_reminders():
    while True:
        ...
        for user_id, reminder_time in list(REMINDER_TIMES.items()):
            ...
            card_available = not last_request_time or (now - last_request_time >= timedelta(days=1))  # Для теста 1 минута
...
async def handle_card_request(message: types.Message, state: FSMContext):
    ...
    if user_id in LAST_REQUEST and now - LAST_REQUEST[user_id] < timedelta(days=1):  # Для теста 1 минута
    ...
    text = f"{name}, давай сделаем это осознанно! 🌿 Подумай: какой вопрос ты хочешь задать карте? Нажми кнопку, когда будешь готов!" if name else "Давай сделаем это осознанно! 🌿 Подумай: какой вопрос ты хочешь задать карте? Нажми кнопку, когда будешь готов!"
