...
def make_user_profile_handler(db, logger):
    async def wrapped_handler(message: types.Message, state: FSMContext):
        ...
        text = (
            ...
            f"<i>Этот профиль помогает мне лучше понимать контекст твоих запросов и адаптировать уточняющие вопросы во время работы с картами.</i>"
        )
...
def make_feedback_handler(db, logger):
    async def wrapped_handler(message: types.Message, state: FSMContext):
        ...
        name = db.get_user(user_id)["name"]
        text = (
            f"{name}, очешь поделиться идеей, как сделать меня лучше, или рассказать о проблеме? Я внимательно читаю все сообщения! Можешь написать здесь все, что думаешь."
            if name
            else "Хочешь поделиться идеей, как сделать меня лучше, или рассказать о проблеме? Я внимательно читаю все сообщения! Можешь написать здесь все, что думаешь."
        )
