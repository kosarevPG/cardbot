...
async def get_grok_question(user_id, user_request, user_response, feedback_type, step=1, previous_responses=None, db=None):
    ...
    system_prompt = (
        ...
        f"Если пользователь обычно отвечает кратко (средняя длина ответа ~{avg_resp_len:.0f} симв.), задай более конкретный, возможно, закрытый вопрос ('Что именно вызывает это чувство?', 'Какой аспект карты связан с этим?'). \nЕсли отвечает развернуто - можно задать более открытый ('Как это перекликается с твоим опытом?', 'Что эта ассоциация говорит о твоих потребностях?'). \n"
...
async def get_grok_supportive_message(user_id, db=None):
    ...
    if recharge_method:
        system_prompt_optional = f" Известно, что ему обычно помогает восстанавливаться: {recharge_method}. \nМожно мягко напомнить об этом или похожих способах заботы о себе, если это уместно."
    ...
    user_prompt = f"Пользователь {name} сообщил, что его ресурсное состояние сейчас низкое (😔). \nНапиши для него поддерживающее сообщение."
    ...
    payload = {
        ...
        "stream": False,
        "temperature": 0.6 # Чуть выше для более душевного ответа 
    }
