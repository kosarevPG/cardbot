...
async def finish_author_test(message: types.Message, state: FSMContext, db: Database, user: types.User | None = None) -> None:
    ...
    if zone == "GREEN":
        result_text = (
            ...
            "• Сопровождение на всех этапах создания продукта от идеи до продажи.\n"
            "\n"
            "✅ Приглашаю познакомиться на онлайн-встрече и проведем стратегическую сессию по поиску вашей идеи.\n"
        )
        ...
        result_text = (
            ...
            "\n"
            "⭐️ По результатам диагностики я вижу у вас есть огромный потенциал к авторству, но сейчас есть факторы, которые могут помешать вам дойти до результата в наставничестве.\n"
            "\n"
            "🌼 Я рекомендую вам пройти подготовительный этап самостоятельно и для этого я приготовила рабочие материалы.\n"
            "\n"
            "💛 После, вы обязательно сможете вернуться к тесту снова. Так вы сэкономите деньги, нервы и получите лучший результат, но чуть позже.\n"
        )
        ...
        result_text = (
            ...
            "\n"
            "🌺 По результатам теста, сейчас наставничество по созданию МАК-карт / Т-игр будет для вас преждевременным и может привести к разочарованию.\n"
            "\n"
            "🔥 И я хочу уберечь вас от этого, ведь в будущем, вы можете создать потрясающий авторский продукт!\n"
            "\n"
