...
async def finish_author_test(message: types.Message, state: FSMContext, db: Database) -> None:
    ...
    if zone == "GREEN":
        ...
        # Две кнопки действий для GREEN: заявка + вопрос (ссылка в личку Изольде с prefill).
        waitlist_text = (
            "Привет, Изольда! Я прошёл(ла) тест в боте и у меня “зелёная зона”.\n"
            "Хочу оставить заявку (лист ожидания) и уточнить: есть ли сейчас место на сопровождение / ближайшее окно?\n"
            "Коротко обо мне: [имя], [город/часовой пояс].\n"
            "Запрос (1–2 предложения): [вставить].\n"
            "Удобный формат: [переписка/созвон]. Время для связи: [дни/часы].\n"
        )
        question_text = (
            "Привет, Изольда! Я прошёл(ла) тест в боте и у меня “зелёная зона”.\n"
            "Хочу уточнить: есть ли сейчас место на сопровождение / ближайшее окно?\n"
            "Коротко обо мне: [имя], [город/часовой пояс].\n"
            "Запрос (1–2 предложения): [вставить].\n"
            "Удобный формат: [переписка/созвон]. Время для связи: [дни/часы].\n"
        )
        waitlist_url = "https://t.me/TopPsyGame1?text=" + quote(waitlist_text, safe="")
        question_url = "https://t.me/TopPsyGame1?text=" + quote(question_text, safe="")
        green_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Оставить заявку (лист ожидания)", url=waitlist_url)],
            [InlineKeyboardButton(text="💬 Задать вопрос (связаться со мной)", url=question_url)],
        ])
