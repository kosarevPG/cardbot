...
async def draw_card_direct(message: types.Message, state: FSMContext, db: Database, logger_service, user_id: int):
    ...
    card_number = None
    try:
        used_cards = db.get_user_cards(user_id); all_card_files = [f for f in os.listdir(CARDS_DIR) if f.startswith("card_") and f.endswith(".jpg")]
        ...
        if not available_cards: logger.info(f"Card deck reset for user {user_id}..."); db.reset_user_cards(user_id); available_cards = all_cards.copy()
        ...
        card_number = random.choice(available_cards); db.add_user_card(user_id, card_number); await state.update_data(card_number=card_number)
    except Exception as card_logic_err: logger.error(f"Error during card selection logic for user {user_id}: {card_logic_err}", exc_info=True); await message.answer("Произошла ошибка при выборе карты..."); await state.clear(); return

    card_path = os.path.join(CARDS_DIR, f"card_{card_number}.jpg")
    if not os.path.exists(card_path): logger.error(f"Card image file not found: {card_path}..."); await message.answer("Изображение для карты потерялось..."); await state.clear(); return
    try:
        await message.bot.send_chat_action(message.chat.id, 'upload_photo'); await message.answer_photo(types.FSInputFile(card_path), protect_content=True)
...
async def ask_grok_question(message: types.Message, state: FSMContext, db: Database, logger_service, step: int, user_id: int):
    ...
    await state.update_data({f"grok_question_{step}": grok_question}); await logger_service.log_action(user_id, "grok_question_asked", {"step": step, "question": grok_question}) # Лог с правильным ID
...
async def process_first_grok_response(message: types.Message, state: FSMContext, db: Database, logger_service):
    ...
    user_id = message.from_user.id # <<< ID пользователя из его сообщения
    first_response = message.text.strip(); data = await state.get_data(); first_grok_question = data.get("grok_question_1", "N/A"); card_number = data.get("card_number", "N/A"); user_request = data.get("user_request", "")
...
async def process_second_grok_response(message: types.Message, state: FSMContext, db: Database, logger_service):
    ...
    user_id = message.from_user.id # <<< ID пользователя из его сообщения
    second_response = message.text.strip(); data = await state.get_data(); second_grok_question = data.get("grok_question_2", "N/A"); card_number = data.get("card_number", "N/A"); user_request = data.get("user_request", "")
...
async def process_third_grok_response(message: types.Message, state: FSMContext, db: Database, logger_service):
    ...
    user_id = message.from_user.id # <<< ID пользователя из его сообщения
    third_response = message.text.strip(); data = await state.get_data(); third_grok_question = data.get("grok_question_3", "N/A"); card_number = data.get("card_number", "N/A"); user_request = data.get("user_request", "")
    ...
    # Передаем user_id явно
    await finish_interaction_flow(message, state, db, logger_service, user_id=user_id)
...
async def generate_and_send_summary(message: types.Message, state: FSMContext, db: Database, logger_service):
    ...
    # Определяем user_id в зависимости от того, вызвано из хендлера сообщения или колбэка
    if isinstance(message.from_user, types.User): # Если это объект Message
         user_id = message.from_user.id
    else: # Иначе предполагаем, что это Message из CallbackQuery (хотя лучше передавать ID явно)
         # На всякий случай, если message не имеет from_user, достаем из state (хотя это плохая практика)
         data = await state.get_data()
         user_id = data.get('_user_id_from_callback') # Предполгаем, что сохранили его в state
         if not user_id: # Fallback, если ID не найден
              logger.error("Could not determine user_id in generate_and_send_summary")
              return
...
# --- Шаг 7: Завершение (ИЗМЕНЕНА сигнатура) ---
async def finish_interaction_flow(message: types.Message, state: FSMContext, db: Database, logger_service, user_id: int):
    """
    ...
    name = user_db_data.get("name") or ""; name = name.strip() if isinstance(name, str) else ""
    data = await state.get_data(); initial_resource = data.get("initial_resource", "неизвестно")
...
async def process_final_resource_callback(callback: types.CallbackQuery, state: FSMContext, db: Database, logger_service):
    ...
    resource_choice_key = callback.data; resource_choice_label = RESOURCE_LEVELS.get(resource_choice_key, "Неизвестно")
    await state.update_data(final_resource=resource_choice_label); await logger_service.log_action(user_id, "final_resource_selected", {"resource": resource_choice_label}) # Лог с правильным ID
...
async def process_recharge_method(message: types.Message, state: FSMContext, db: Database, logger_service):
    ...
    user_id = message.from_user.id # <<< ID пользователя из его сообщения
    recharge_method_text = message.text.strip(); user_db_data = db.get_user(user_id) or {}; name = user_db_data.get("name") or ""; name = name.strip() if isinstance(name, str) else ""
...
async def show_final_feedback_and_menu(message: types.Message, state: FSMContext, db: Database, logger_service, user_id: int):
    ...
    """
    # Используем переданный user_id
    user_db_data = db.get_user(user_id) or {}; name = user_db_data.get("name") or ""; name = name.strip() if isinstance(name, str) else ""; data = await state.get_data(); card_number = data.get("card_number", 0)
    ...
    try:
        ...
        if final_profile_data: db.update_user_profile(user_id, final_profile_data); logger.info(f"Final profile data (resources) saved for user {user_id} before state clear.") # Лог с правильным ID
    ...
    logger.info(f"Clearing state for user {user_id} after card session. Current state: {current_state_before_clear}. Data: {data}") # Лог с правильным ID
    await state.clear()
