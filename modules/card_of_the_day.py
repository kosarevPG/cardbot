...
async def handle_card_request(message: types.Message, state: FSMContext, db, logger_service):
    ...
    text = f"{name}, давай вытянем карту осознанно! 🌿 Напиши свой вопрос или нажми 'Вытянуть карту', чтобы сфокусироваться на дне!" if name else "Давай вытянем карту осознанно! 🌿 Напиши свой вопрос или нажми 'Вытянуть карту', чтобы сфокусироваться на дне!"
...
async def draw_card(callback: types.CallbackQuery, state: FSMContext, db, logger_service):
    ...
    if os.path.exists(card_path):
        try:
            ...
            text = f"{name}, как этот образ откликается тебе? На что он похож в твоей жизни сейчас? Напиши свои мысли!" if name else "Как этот образ откликается тебе? На что он похож в твоей жизни сейчас? Напиши свои мысли!"
...
async def process_request_text(message: types.Message, state: FSMContext, db, logger_service):
    ...
    if os.path.exists(card_path):
        try:
            ...
            text = f"{name}, как этот образ отвечает на твой запрос '{request_text}'? Напиши свои мысли!" if name else f"Как этот образ отвечает на твой запрос '{request_text}'? Напиши свои мысли!"
...
async def process_third_grok_response(message: types.Message, state: FSMContext, db, logger_service):
    ...
    if summary_text and not summary_text.startswith("Ошибка") and not summary_text.startswith("К сожалению") and not summary_text.startswith("Не получилось"):
         await message.answer(f"✨ Небольшое резюме твоих размышлений:\n{summary_text}")
    ...
    feedback_text = f"{name}, как оцениваешь нашу сегодняшнюю беседу? Помогла ли она тебе глубже взглянуть на свой запрос?" if name else "Как оцениваешь нашу сегодняшнюю беседу? Помогла ли она тебе глубже взглянуть на свой запрос?"
    ...
    try:
        ...
        if len(parts) >= 4 and parts[0] == 'feedback' and parts[1] == 'v2':
            ...
            if feedback_type == "helped":
                text = "Отлично! Рад, что наша беседа помогла тебе продвинуться. 😊 Жду тебя завтра!"
                ...
                text = f"{name}, понимаю. Иногда хочется копнуть еще глубже. Буду благодарен, если поделишься мыслями, как я могу стать лучше, через команду /feedback." if name else "Понимаю. Иногда хочется копнуть еще глубже. Буду благодарен, если поделишься мыслями, как я могу стать лучше, через команду /feedback."
            else:
